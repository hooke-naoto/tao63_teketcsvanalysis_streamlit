...
def build_chain(rows, actor_col, partner_col, status_col):
    ...
    chain_str = " -> ".join(dedup) if dedup else ""
    return chain_str, final_owner

def extract_seat_token(x):
    s = str(x)
    # 「○階△列×番」などを抽出して1-02-12形式に変換
    m = re.search(r"(\d+)階.*?(\d+)列.*?(\d+)番", s)
    if m:
        return f"{int(m.group(1))}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"
    # 「S席1-12」などを抽出
    m = re.search(r"(\d+)[^\d]+(\d+)", s)
    if m:
        return f"1-{int(m.group(1)):02d}-{int(m.group(2)):02d}"
    return s
...
time_col     = guess_col(df.columns, ["購入日時"])
